...
function App(): React.JSX.Element {
  ...
  function renderScreen(): React.JSX.Element {
    ...
    if (screen === 'pages') return <PagesScreen documentTitle={document.title} pages={document.pages} onBack={() => setScreen('home')} onContinueScan={beginScan} onImport={importPhotos} onExport={() => setScreen('export')} onRename={title => touchDocument({...document, title})} onDeletePage={page => { touchDocument({...document, pages: document.pages.filter(item => item.id !== page.id)}); const paths = [page.originalImagePath, page.processedImagePath, page.thumbnailPath].filter((path): path is string => Boolean(path)); void Promise.all([...new Set(paths)].map(path => fileStore.removeFile(path).catch(() => undefined))); }} onDeleteDocument={() => { void fileStore.deleteWorkspace(document.id).catch(() => undefined); void fileStore.deleteDocument(document.id).catch(() => undefined); setDocument(createEmptyDocument(document.id, new Date().toISOString())); setScreen('home'); }} onReorder={pages => touchDocument({...document, pages})} />;
