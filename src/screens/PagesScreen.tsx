...
import {getPageCardWidth, MAX_CONTROL_TEXT_SCALE, MAX_TEXT_SCALE, MAX_TITLE_TEXT_SCALE, useResponsiveMetrics} from '../theme/responsive';
...
function PagesScreen({
  ...
  const pageCardWidth = getPageCardWidth(
  ...
  return (
    <View style={styles.screen}>
        ...
        <View style={styles.headerCopy}>
          ...
          {editingTitle ? (
            <TextInput
              ...
              returnKeyType="done"
              maxFontSizeMultiplier={MAX_TITLE_TEXT_SCALE}
              style={{...styles.titleInput, fontSize: metrics.titleSize, lineHeight: metrics.titleLineHeight}}
              value={title}
            />
          ) : (
            <Pressable accessibilityLabel="编辑文档标题" accessibilityRole="button" onPress={() => setEditingTitle(true)}>
              <Text accessibilityRole="header" maxFontSizeMultiplier={MAX_TITLE_TEXT_SCALE} numberOfLines={1} style={{...styles.title, fontSize: metrics.titleSize, lineHeight: metrics.titleLineHeight}}>{title}</Text>
            </Pressable>
          )}
          <Text maxFontSizeMultiplier={MAX_TEXT_SCALE} numberOfLines={2} style={styles.updated}>{visiblePages.length} 页 · 长按或按钮调整顺序</Text>
        </View>
      ...
      <ScrollView contentContainerStyle={[styles.content, {paddingHorizontal: metrics.horizontalInset}]}>
        {visiblePages.length === 0 ? (
          <View style={styles.empty}>
            <Text maxFontSizeMultiplier={MAX_TEXT_SCALE} style={styles.emptyTitle}>还没有页面</Text>
            <Text maxFontSizeMultiplier={MAX_TEXT_SCALE} style={styles.emptyText}>再拍或从相册导入页面。</Text>
          </View>
        ) : (
          <View style={[styles.grid, {columnGap: metrics.contentGap, rowGap: metrics.contentGap}]}>
            {visiblePages.map((page, index) => (
              <Surface elevation={2} key={page.id} style={[styles.pageItem, {width: pageCardWidth}, index === 0 && styles.selectedPage]}>
                <View style={styles.pageNumber}><Text maxFontSizeMultiplier={MAX_CONTROL_TEXT_SCALE} style={styles.pageNumberText}>{index + 1}</Text></View>
                ...
                <Text maxFontSizeMultiplier={MAX_CONTROL_TEXT_SCALE} style={styles.pageMeta} numberOfLines={1}>{pageName(index)} · {modeLabel(page.enhanceMode)}</Text>
                <View style={styles.pageActions}>
                  <Pressable accessibilityLabel={`预览第 ${index + 1} 页`} onPress={() => setPreview({page, index})} style={styles.miniButton}><Text maxFontSizeMultiplier={MAX_CONTROL_TEXT_SCALE} numberOfLines={1} style={styles.miniButtonText}>预览</Text></Pressable>
                  <Pressable accessibilityLabel={`第 ${index + 1} 页上移`} disabled={index === 0} onPress={() => movePage(index, -1)} style={[styles.miniButton, index === 0 && styles.disabledMiniButton]}><Text maxFontSizeMultiplier={MAX_CONTROL_TEXT_SCALE} numberOfLines={1} style={styles.miniButtonText}>上移</Text></Pressable>
                  <Pressable accessibilityLabel={`删除第 ${index + 1} 页`} onPress={() => confirmDelete(page)} style={[styles.miniButton, styles.deleteMiniButton]}><Text maxFontSizeMultiplier={MAX_CONTROL_TEXT_SCALE} numberOfLines={1} style={styles.deleteMiniButtonText}>删除</Text></Pressable>
                </View>
              </Surface>
            ))}
          </View>
        ...
        <View style={styles.previewOverlay}>
          <View style={styles.previewDialog}>
            <View style={styles.previewHeader}>
              <Text maxFontSizeMultiplier={MAX_TEXT_SCALE} style={styles.previewTitle}>{preview ? `第 ${preview.index + 1} 页` : '页面预览'}</Text>
              <Pressable accessibilityLabel="关闭页面预览" accessibilityRole="button" onPress={() => setPreview(undefined)} style={styles.previewCloseButton}>
                <Text maxFontSizeMultiplier={MAX_CONTROL_TEXT_SCALE} style={styles.previewCloseText}>关闭</Text>
              </Pressable>
            </View>
            {preview?.page.processedImagePath || preview?.page.originalImagePath ? (
              ...
              <View accessibilityLabel={`第 ${(preview?.index ?? 0) + 1} 页大图预览`} style={styles.previewEmpty}>
                <Text maxFontSizeMultiplier={MAX_TEXT_SCALE} style={styles.previewEmptyText}>暂无图片预览</Text>
              </View>
            )}
          </View>
        </View>
      ...
      <View style={[styles.bottom, {gap: metrics.contentGap, paddingBottom: insets.bottom + theme.spacing.md, paddingHorizontal: metrics.horizontalInset}]}>
        <Button accessibilityLabel="相册导入" compact maxFontSizeMultiplier={MAX_CONTROL_TEXT_SCALE} mode="outlined" onPress={onImport} textColor={theme.colors.inkPrimary} style={styles.bottomButton} contentStyle={styles.bottomButtonContent}>相册导入</Button>
        <Button accessibilityLabel="导出 PDF" compact maxFontSizeMultiplier={MAX_CONTROL_TEXT_SCALE} mode="contained" onPress={onExport} buttonColor={theme.colors.actionPrimary} style={styles.exportButton} contentStyle={styles.bottomButtonContent}>导出 PDF</Button>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  header: {alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', minHeight: 96, paddingBottom: theme.spacing.sm},
  ...
  title: {color: theme.colors.inkPrimary, fontSize: theme.typography.title, fontWeight: '800', letterSpacing: 0, lineHeight: 36, textAlign: 'center'},
  titleInput: {borderBottomColor: theme.colors.actionPrimary, borderBottomWidth: 3, color: theme.colors.inkPrimary, fontSize: theme.typography.title, fontWeight: '800', minHeight: 44, paddingHorizontal: 0, paddingVertical: 0, textAlign: 'center'},
  updated: {color: theme.colors.inkSecondary, fontSize: theme.typography.caption, fontWeight: '600', lineHeight: 18, marginTop: theme.spacing.xs, textAlign: 'center'},
  ...
  content: {backgroundColor: theme.colors.surfaceDefault, flexGrow: 1, paddingTop: theme.spacing.sm, paddingBottom: theme.spacing.md},
  grid: {flexDirection: 'row', flexWrap: 'wrap'},
  pageItem: {backgroundColor: theme.colors.surfaceWarm, borderColor: theme.colors.inkPrimary, borderRadius: theme.radii.md, borderWidth: 2, elevation: 3, padding: theme.spacing.sm, shadowColor: theme.colors.shadowInk, shadowOffset: {height: 5, width: 5}, shadowOpacity: 1, shadowRadius: 0},
  ...
  miniButton: {alignItems: 'center', borderColor: theme.colors.inkPrimary, borderRadius: theme.radii.sm, borderWidth: 2, flex: 1, minHeight: 38, justifyContent: 'center', minWidth: 0},
  ...
  bottom: {backgroundColor: theme.colors.surfaceWarm, borderTopColor: theme.colors.inkPrimary, borderTopWidth: 3, flexDirection: 'row', paddingTop: theme.spacing.md},
